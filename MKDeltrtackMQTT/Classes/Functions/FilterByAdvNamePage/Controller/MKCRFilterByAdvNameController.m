//
//  MKCRFilterByAdvNameController.m
...
@interface MKCRFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKCRFilterByAdvNameModel *dataModel;

@end

@implementation MKCRFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKCRFilterByAdvNameController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"BLE Name Whitelist";
    [self.rightButton setImage:LOADICON(@"MKDeltrtackMQTT", @"MKCRFilterByAdvNameController", @"cr_saveIcon.png") forState:UIControlStateNormal];
...
- (MKCRFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKCRFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
